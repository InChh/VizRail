...
#include <cstdarg>
#include <string>
// The <string> header defines __cpp_lib_string_view, if std::string_view is available
// std::string_view requires cpp++17 or higher
//
...
#define ENABLE_ACSTRING_CSTRING_OPERATORS
    ...
    explicit AcString(const std::wstring& str);
...
// Input pointer must not be null, and return pointer will not be null
//
...
#ifdef __cpp_lib_string_view
inline AcString::AcString(std::wstring_view in)
// The result of wstring_view::data is not a c-string and may not be null-terminated
    : AcString(in.data(), static_cast<unsigned int>(in.size()))
{
}

inline AcString& AcString::operator=(std::wstring_view in)
{
    *this = AcString (in);
    return *this;
}
...
inline AcString::AcString(const std::wstring& in)
    : AcString(in.c_str())
{
}
